...
//	return 0;
	...
	if (inet_pton(AF_INET, hosttoip(argv[1]), &servaddr.sin_addr) <= 0) {
...
//	return 0;
	...
	sentpk = sendpk(hspk(itov(754), ctos(argv[1]), 25565, itov(2)), sockfd);
...
packet_t decpk(packet_t pk, unsigned char *key, EVP_CIPHER_CTX *ctx)
{
	...
	ret->len = outl;
	/*
	fprintf(stderr, "outputting decrypted packet\n");
...
int readpk(packet_t pk)
{
	...
	char *data = pk->data;
	int len  = pk->len;
	int pklen = vtoi_raw(data); /* without packet lenth bytes */
	while ((*data++  & 0x80) > 0)
		;
...
unsigned char *encrypt(unsigned  char *msg, long len, unsigned const char *pubkey, long n)
{
	...
	*rsap = RSA_new();
	//write(1, pubkey, n);
	RSA *rsa = d2i_RSA_PUBKEY(rsap, &pubkey, n);
	if (rsa == NULL) {
	...
	fprintf(stderr, "encrypted shared secret\n");
	//write(1, ret, 16);
	return ret;
